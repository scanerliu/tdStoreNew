...
function getDistrictSelections(level, selectedDistrictId){
	// 选择第三级时，无需异步加载
	if(level != 3){
		var url = basePath+"/admin/district/getDistrictSelections";
		var loadData = {"level":level, "selectedDistrictId":selectedDistrictId};
		// 根据地区级别确定加载区域, level=0为首次进入列表页，未选择下拉框的情况。level!=0的值代表选择了第level级下拉框
		if(level == 0){
			$("#firstDistrictLevel").load(url,loadData);
			$("#secondDistrictLevel").html("");
			$("#thirdDistrictLevel").html("");
		}else if(level == 1){
			$("#region_id").val(selectedDistrictId);
			if(selectedDistrictId > 0){
				$("#secondDistrictLevel").load(url,loadData);
				$("#thirdDistrictLevel").html("");
			}else{
				$("#secondDistrictLevel").html("");
				$("#thirdDistrictLevel").html("");
			...
			if(selectedDistrictId > 0){
				$("#region_id").val(selectedDistrictId);
				$("#thirdDistrictLevel").load(url,loadData);				
			}else{
				var parent_id = $("#district_id_" + (level-1)).val();
